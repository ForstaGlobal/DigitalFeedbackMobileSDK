...
class DefaultLayoutQuestionSection: DefaultLayoutSection {
  ...
  init(question: Question) {
    ...
    items = createItems(question: self.question)
  }
  ...
  private func updateValidation(question: DefaultQuestion, errors: [ValidationQuestionError]) {
    ...
    guard let iconLabelItem = item as? DefaultLayoutIconLabel else {
      if errors.count > 0 {
        ...
        items.insert(DefaultLayoutIconLabel(errors: errors), at: validationIndex.row)
        delegate?.onRefreshRow(add: [validationIndex], refresh: [], remove: [])
      }
      return
    }
    ...
    if iconLabelItem.errors.count > 0 { // Validation already exist
      if errors.count == 0 { // No more error. Clear validation
        items.remove(at: validationIndex.row)
    ...
    delegate?.onRefreshRow(add: addRow, refresh: refreshRow, remove: removeRow)
  }
  ...
  func onRadioClicked(answer: QuestionAnswer) {
    if let question = question as? SingleQuestion {
      ...
      delegate?.onRefreshRow(add: [],
                             refresh: DefaultLayoutItemUtils.getSingleAnswerIndexList(section: section, question: question, excludeCode: answer.code),
                             remove: [])
    }
  }
  ...
  private func validate(question: DefaultQuestion) {
    // For dynamic validation, you can run validation before commit survey answers
    // NOTE: Quick implementation. do / catch is better practice
    // let errors = try! question.validate()
    // updateValidation(question: question, errors: errors)
  }
}
